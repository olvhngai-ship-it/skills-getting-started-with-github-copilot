document.addEventListener("DOMContentLoaded", () => {
  ...
  async function refreshActivities(selectedActivity = activitySelect.value) {
    ...
    if (selectedActivity && activitySelect.querySelector(`option[value="${CSS.escape(selectedActivity)}"]`)) {
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        await refreshActivities(activity);
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
        messageDiv.className = "error";
      }
